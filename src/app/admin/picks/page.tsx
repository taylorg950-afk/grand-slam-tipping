...
export default async function AdminPicksStatusPage() {
  ...
  if (!tournament) {
    return (
      <div className="space-y-2">
        <h1 className="text-xl font-semibold">Picks status</h1>
        <p className="text-sm text-zinc-500">No active tournament.</p>
      </div>
    )
  }
  ...
  if (!activeRound) {
    return (
      <div className="space-y-2">
        <h1 className="text-xl font-semibold">Picks status</h1>
        <p className="text-sm text-zinc-500">No rounds with matches yet for {tournament.name}.</p>
      </div>
    )
  }
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-100">
            {rows.map(r => {
              ...
              return (
                <tr key={r.id} className={complete ? '' : 'bg-amber-50/40'}>
                  ...
                  <td className="px-4 py-2.5 text-right tabular-nums">
                    {r.stillToPick > 0 ? (
                      <span className="font-semibold text-amber-700">{r.stillToPick}</span>
                    ) : (
