...
const ACCENT_COLOUR = '#E8500A'
const OTHERS_COLOUR = '#cbd5e1'

function buildSeriesConfig(data: CumulativePointsData[]) {
...
export default function CumulativePointsChart({ data, currentUserName }: Props) {
  const series = buildSeriesConfig(data)
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 16px', marginBottom: 16 }}>
        {namedSeries.map(s => (
          <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{
              fontSize: 13,
              color: s.name === leader || s.name === currentUserName ? 'var(--text-primary)' : 'var(--text-secondary)',
        ...
        {otherCount > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>+ {otherCount} others</span>
          </div>
        )}
      </div>
      ...
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: -16 }}>
          <CartesianGrid vertical={false} stroke="var(--border-default)" strokeOpacity={0.6} />
          <XAxis
            dataKey="round"
